...
module.factory 'readXls', () ->
  {
    ...
    load: (file, filename) ->
      ...
      dendrite.spine_means = {
        length: stats.mean _.map(dendrite.spines, (spine) -> spine.length)
        diameter: stats.mean _.map(dendrite.spines, (spine) -> spine.diameter)
        distance: stats.mean _.map(dendrite.spines, (spine) -> spine.distance)
        length_to_center: stats.mean _.map(dendrite.spines, (spine) -> spine.length_to_center)
      }
